...
test('builds URL for resource without path params', () => {
  ...
  expect(url).toBe('https://api-testnet.fluentscan.xyz/api/v2/config/backend-version');
});

test('builds URL for resource with path params', () => {
  const url = buildUrl('general:block', { height_or_hash: '42' });
  expect(url).toBe('https://api-testnet.fluentscan.xyz/api/v2/blocks/42');
});
...
describe('falsy query parameters', () => {
  test('leaves "false" as query parameter', () => {
    ...
    expect(url).toBe('https://api-testnet.fluentscan.xyz/api/v2/blocks/42?includeTx=false');
  });

  test('leaves "null" as query parameter', () => {
    const url = buildUrl('general:block', { height_or_hash: '42' }, { includeTx: null });
    expect(url).toBe('https://api-testnet.fluentscan.xyz/api/v2/blocks/42?includeTx=null');
  });
...
test('builds URL with array-like query parameters', () => {
  ...
  expect(url).toBe('https://api-testnet.fluentscan.xyz/api/v2/blocks/42?includeTx=0x11%2C0x22&sort=asc');
});
